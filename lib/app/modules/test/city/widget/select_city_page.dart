...
class SelectCityPage extends StatefulWidget {
  ...
  /// 热门推荐城市列表
  final List<CityModel> hotCityList;

  /// 单选项 点击的回调
  final ValueChanged<CityModel>? onValueChanged;
...
class _SelectCityPageState extends State<SelectCityPage> {
  ...
  void _loadData() async {
    if (widget.cityList == null || widget.cityList!.isEmpty) {
      //加载城市列表
      rootBundle.loadString('assets/jsons/china.json').then((value) {
        Map countyMap = json.decode(value);
        List list = countyMap['china'];
        for (var item in list) {
          _cityList.add(CityModel(name: item['name']));
        }
  ...
  Widget _buildHeader() {
    List<CityModel> hotCityList = widget.hotCityList;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(title: Text(widget.appBarTitle ?? '城市选择')),
        body: Container(
  ...
  Widget _buildCityList() {
    ...
    return Expanded(
        ...
        child: AzListView(
          data: _cityList,
          itemBuilder: (context, model) => _buildListItem(model as CityModel),
  ...
  Widget _buildSearchResultList(String searchText) {
    List<CityModel> cList = _searchCityList(searchText);
  ...
  ///获取城市搜索结果
  List<CityModel> _searchCityList(String searchText) {
    List<CityModel> cList = [];
    for (int index = 0; index < _cityList.length; index++) {
      CityModel cInfo = _cityList[index];
